...
function hasCallDescendantOrSelf(node) {
	if (node.type === 'CallExpression') return true;
	for (const childNode of getSignificantChildren(node)) {
		if (hasCallDescendantOrSelf(childNode)) return true;
	}
	return false;
}

function hasArrayAccessDescendantOrSelf(node) {
	if (node.type === 'ArrayElement') return true;
	for (const childNode of getSignificantChildren(node)) {
		if (hasArrayAccessDescendantOrSelf(childNode)) return true;
	}
	return false;
}
...
function* walkSubTree(tree, rootNode) {
	const ENTER_NODE = {enter: true, leave: false, direction: 1, node: null};
	const LEAVE_NODE = {enter: false, leave: true, direction: 0, node: null};
	...
	while (!reachedRoot) {
		...
		if (cursor.currentNode === rootNode) {
			break;
		}
...
module.exports = {
	...
	hasCallDescendantOrSelf,
	hasArrayAccessDescendantOrSelf,
